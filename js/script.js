...
function obtenerHorario() {
  ...
  const abierto = minutos >= 720 && minutos < 1080;
...
/* ---------- Mensaje de WhatsApp con todos los datos ---------- */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('a[data-wa]').forEach(el => {
    el.addEventListener('click', e => {
      if (el.hasAttribute('data-comp') || el.hasAttribute('data-abre-carrito')) return;
  ...
  compGrid.addEventListener('click', e => {
    ...
    if (!chip) return;
    if (negocioAbierto) agregarAlCarrito(chip.dataset.compAdd, parseInt(chip.dataset.precio, 10), 1);
    else toast(mensajeCerrado());
  });
  ...
  document.getElementById('cart-send').addEventListener('click', e => {
    ...
    [nombre, telefono].forEach(el => el.classList.remove('invalid'));
    ...
    if (digitos.length !== 10) {
      e.preventDefault(); mostrarPanel('panel-datos'); marcar(telefono);
    ...
    if (metodoPago === 'efectivo') {
      ...
      if (!monto) {
        e.preventDefault(); mostrarPanel('panel-datos'); marcar(montoInput);
        toast('💵 Indica con cuánto pagarás al recibir'); return;
      }
      if (monto < totalFinal) {
        e.preventDefault(); marcar(montoInput);
